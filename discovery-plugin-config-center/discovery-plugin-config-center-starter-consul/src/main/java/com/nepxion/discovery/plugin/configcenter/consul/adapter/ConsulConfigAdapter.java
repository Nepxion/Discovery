...
public class ConsulConfigAdapter extends ConfigAdapter {
    ...
    private void subscribeConfig(boolean globalConfig) {
        ...
        try {
            consulOperation.subscribeConfig(group, dataId, globalConfig, new ConsulSubscribeCallback() {
                @Override
    ...
    public void unsubscribeConfig() {
        unsubscribeConfig(false);
        unsubscribeConfig(true);
    }

    private void unsubscribeConfig(boolean globalConfig) {
        String group = getGroup();
        String dataId = getDataId(globalConfig);
        ...
        try {
            consulOperation.unsubscribeConfig(group, dataId, globalConfig);
        } catch (Exception e) {
