...
### 区域维度的多活单元化概念
异地多活，主要是为了提升系统的容灾能力，比如单机房遭遇地震、火灾、网络故障、断电等不可抗因素，都有可能造成整个机房瘫痪

基于向外提供数据和服务实时性和连续性的要求，需要在不同城市建立独立的数据中心，并搭建配套的网关和服务集群，消息队列，数据库等，当某个城市的机房崩溃，则通过SLB等最高层的执行流量调拨，从一个城市切换到另一个城市，让外界感知服务永远处于有效状态

要进行多活建设，需要梳理企业内的服务。服务从多活的角度，一般分为三种类型
...
① 部署方案
- 单元区可以有很多个，进行对等镜像部署方式
- 非单元区域只有一个，在就近原则下，一般部署在和常用的单元区同一个物理空间内

> 最古典的多活方案，不建议出现单机房部署的非单元区。受制和妥协于企业历史包袱和现状，无法进行多活单元化拆分，且对数据一致性要求很高，故出现了单点的全局服务，还有，因为成本因素，普通服务也不考虑做多活单元化拆分。所以，需要保持非单元区服务高可用性

② 注册方案
- 所有API网关、单元区服务、非单元区服务都注册到同一个物理空间下的注册中心
- 不同物理空间下的注册中心需要双向同步

③ 网关方案
- API网关属于单元区的范畴，一个单元区需要部署一个API网关的集群
- API网关具有跨区域路由的功能，即单元一区的API网关可以路由到单元二区的服务，也可以路由到任何非单元区的服务

④ 调用方案
- 单元区之间服务调用是隔离的，两个单元区的服务不能跨区域调用
- 非单元区有回溯功能，当调用链为`单元区 -> 非单元区 -> 单元区`，非单元区再调回单元区的时候，仍旧选择发起调用的那个单元区，即不会出现类似`单元区1 -> 非单元区 -> 单元区2`的情况，原则是`从哪里来回哪里去`

⑤ 切流方案
 - 单元一区因为灾难性的事故导致崩溃，通过前置的SLB通过改变Header`n-d-region`切换到单元二区

⑥ 路由（故障）转移方案
- 单元区内服务相互调用，如果找不到匹配`n-d-region`的服务实例，直接失败，达到屏蔽其它单元区的目的
- 非单元区内服务相互调用，由于不存在匹配`n-d-region`的服务实例，路由转移到自己所在区的服务
- 单元区服务调用非单元区服务，由于非单元区不存在匹配`n-d-region`的服务实例，路由转移到非单元区的服务
- 非单元区服务调用单元区服务，按照匹配`n-d-region`的服务实例的原则，路由转移匹配`n-d-region`的单元区，达到`从哪里来回哪里去`的目的
