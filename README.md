...
- 服务限流熔断降级权限，定制阿里巴巴Sentinel，有机整合灰度路由，扩展LimitApp的机制，通过动态的Http Header方式实现组合式熔断，包括基于组、基于服务名、基于版本、基于区域等熔断机制，支持自定义任意的业务参数组合实现该功能。支持原生的流控规则、降级规则、授权规则、系统规则、热点参数流控规则
