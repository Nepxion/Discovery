...
- 灰度发布中
  - __新增用作灰度发布的网关(V1.1)，指向服务A(V1.1)->服务B(V1.1)__
  - __灰度网关(V1.1)发布到服务注册发现中心，但禁止被服务发现，网关外的调用进来无法负载均衡到网关(V1.1)上__
  - 在灰度网关(V1.1)->服务A(V1.1)->服务B(V1.1)这条调用路径做灰度测试
  - __灰度测试成功后，把网关(V1.0)指向服务A(V1.1)->服务B(V1.1)__
- 灰度发布后  
  - __下线服务A(V1.0)，服务B(V1.0)，灰度成功__
