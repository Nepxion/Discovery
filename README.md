...
- 基于Header传递的全链路灰度路由，网关为路由触发点。采用配置中心配置路由策略映射在网关过滤器中植入Header信息而实现，路由策略传递到全链路服务中。主要包括
    ...
    - 前端触发路由
    - 网关过滤器触发路由。包括基于Header、Query Parameter和Cookie为触发的路由
    - 负载均衡策略类触发路由。包括基于Header、Query Parameter、Cookie和RPC Method为触发的路由
    - 并行路由下的版本优选路由
    - 异步场景下的触发路由
- 基于动态变更元数据的全链路灰度路由
- 基于全局订阅式的全链路灰度路由
...
| --- | --- | --- | --- | --- | --- |
| 6.3.1 | master | ![](http://nepxion.gitee.io/docs/icon-doc/confirm_24.png) | Hoxton.SR5 `↑`<br>Hoxton<br>Greenwich<br>Finchley | 2.3.x.RELEASE<br>2.2.x.RELEASE<br>2.1.x.RELEASE<br>2.0.x.RELEASE | 2.2.x.RELEASE<br>2.2.x.RELEASE<br>2.1.x.RELEASE<br>2.0.x.RELEASE |
...
| 3.20.1 | 3.x.x | ![](http://nepxion.gitee.io/docs/icon-doc/arrow_up_24.png) | Edgware | 1.5.x.RELEASE | 1.5.x.RELEASE |
