...
【模拟场景2】兜底策略，测试网关 -> 服务A -> 服务B全链路调用...
测试结果 : 通过
【模拟场景2】* 测试通过...
-------------------------------------------------
【模拟场景3】第1次蓝绿灰度发布场景，启动蓝绿灰度发布策略...
输入规则策略: 
service:
  - discovery-guide-service-a
  - discovery-guide-service-b
...
【模拟场景3】蓝绿策略，测试网关 -> 服务A -> 服务B全链路调用，Header xyz缺失...
测试结果 : 通过
【模拟场景3】蓝绿策略，测试网关 -> 服务A -> 服务B全链路调用，Header xyz等于1...
测试结果 : 通过
【模拟场景3】蓝绿策略，测试网关 -> 服务A -> 服务B全链路调用，Header xyz等于2...
测试结果 : 通过
【模拟场景3】灰度策略，测试网关 -> 服务A -> 服务B全链路调用，Header xyz缺失...
抽样次数=1500
权重结果偏差值=2%
A服务期望 : 1.0版本权重=100%, 1.1版本权重=0%
B服务期望 : 1.0版本权重=100%, 1.1版本权重=0%
调用结果 : A服务1.0版本权重=100.0%
调用结果 : A服务1.1版本权重=0.0%
调用结果 : B服务1.0版本权重=100.0%
调用结果 : B服务1.1版本权重=0.0%
测试结果 : 通过
【模拟场景3】灰度策略，测试网关 -> 服务A -> 服务B全链路调用，Header xyz等于3...
抽样次数=1500
权重结果偏差值=2%
A服务期望 : 1.0版本权重=90%, 1.1版本权重=10%
B服务期望 : 1.0版本权重=90%, 1.1版本权重=10%
调用结果 : A服务1.0版本权重=89.8%
调用结果 : A服务1.1版本权重=10.2%
调用结果 : B服务1.0版本权重=89.8%
调用结果 : B服务1.1版本权重=10.2%
测试结果 : 通过
【模拟场景3】灰度策略，测试网关 -> 服务A -> 服务B全链路调用，Header xyz等于4...
抽样次数=1500
权重结果偏差值=2%
A服务期望 : 1.0版本权重=70%, 1.1版本权重=30%
B服务期望 : 1.0版本权重=70%, 1.1版本权重=30%
调用结果 : A服务1.0版本权重=70.2667%
调用结果 : A服务1.1版本权重=29.7333%
调用结果 : B服务1.0版本权重=70.2667%
调用结果 : B服务1.1版本权重=29.7333%
测试结果 : 通过
【模拟场景3】* 测试通过...
-------------------------------------------------
【模拟场景4】第1次蓝绿灰度发布场景，重置并停止蓝绿灰度发布策略...
输入规则策略: 无
输出规则策略 : 
<?xml version="1.0" encoding="UTF-8"?>
<rule>
