...
public class DiscoveryGatewayPlugin extends Plugin {
    private final static AgentLogger LOG = AgentLogger.getLogger(DiscoveryGatewayPlugin.class.getName());
    ...
    public void install(TransformTemplate transformTemplate) {
        ...
        transformTemplate.transform(classMatcher, new TransformCallback() {
            ...
            public byte[] doInTransform(ClassLoader classLoader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) {
                try {
                    URLClassLoader urlClassLoader = new URLClassLoader(((URLClassLoader) DiscoveryGatewayPlugin.class.getClassLoader()).getURLs(), classLoader);
                    Object object = Class.forName("com.nepxion.discovery.plugin.strategy.agent.plugin.gateway.GatewayStrategyContextHook", true, urlClassLoader).newInstance();
